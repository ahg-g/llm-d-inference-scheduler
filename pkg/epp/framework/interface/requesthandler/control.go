...
type PreRequest interface {
	plugin.Plugin
	PreRequest(ctx context.Context, request *InferenceRequest, schedulingResult *SchedulingResult)
}
...
type ResponseHeaderProcessor interface {
	plugin.Plugin
	ResponseHeader(ctx context.Context, request *InferenceRequest, response *Response, targetEndpoint *datalayer.EndpointMetadata)
}
...
type ResponseBodyProcessor interface {
	plugin.Plugin
	ResponseBody(ctx context.Context, request *InferenceRequest, response *Response, targetEndpoint *datalayer.EndpointMetadata)
}
...
type DataProducer interface {
	...
	Produce(ctx context.Context, request *InferenceRequest, pods []Endpoint) error
}
...
type Admitter interface {
	...
	AdmitRequest(ctx context.Context, request *InferenceRequest, pods []Endpoint) error
}
