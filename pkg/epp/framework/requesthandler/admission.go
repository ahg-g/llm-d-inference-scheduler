...
// handlers.AdmissionController defines the interface for making admission control decisions.
...
// This interface is now defined in the handlers package to avoid circular dependencies.
...
// Admit implements the handlers.AdmissionController interface for the legacy strategy.
...
// Admit implements the handlers.AdmissionController interface by checking for saturation on sheddable requests first, then
...
type flowControlRequest struct {
	...
	inferenceRequest  *fwkrhapi.InferenceRequest
...
func (r *flowControlRequest) InferenceRequest() *fwkrhapi.InferenceRequest {
