...
var _ fwkrhapi.DataProducer = &executorMockDataProducerPlugin{}
...
func (m *executorMockDataProducerPlugin) Produce(ctx context.Context, request *fwkrhapi.InferenceRequest, endpoints []fwkrhapi.Endpoint) error {
	m.executed = true
	if m.delay > 0 {
		select {
...
func (p *ctxObservingPlugin) Produce(ctx context.Context, _ *fwkrhapi.InferenceRequest, _ []fwkrhapi.Endpoint) error {
	defer p.wg.Done()
	select {
...
func TestDataProducerPluginsWithTimeout_CancelsPluginContext(t *testing.T) {
	...
	err := dataProducerPluginsWithTimeout(
		context.Background(),
		20*time.Millisecond,
		[]fwkrhapi.DataProducer{plugin},
		&fwkrhapi.InferenceRequest{},
		nil,
	)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "fwkrhapi.DataProducer execution timed out")
...
func TestDataProducerPluginsWithTimeout(t *testing.T) {
	testCases := []struct {
		...
		expectErrStr  string
		checkPlugins  func(t *testing.T, plugins []fwkrhapi.DataProducer)
		expectSuccess bool
	}{
		{
		...
		},
		{
			...
			expectErrStr: "fwkrhapi.DataProducer \"p1/mock\" failed: plugin failed",
		},
		{
			...
			expectErrStr: "fwkrhapi.DataProducer execution timed out",
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := dataProducerPluginsWithTimeout(ctx, tc.timeout, tc.plugins, &fwkrhapi.InferenceRequest{}, nil)
...
func (p *dagTestPlugin) Produce(ctx context.Context, request *fwkrhapi.InferenceRequest, endpoints []fwkrhapi.Endpoint) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.execTime = time.Now()
...
func TestExecutePluginsAsDAG(t *testing.T) {
	...
	testCases := []struct {
		...
		plugins   []fwkrhapi.DataProducer
		expectErr bool
		checkFunc func(t *testing.T, plugins []fwkrhapi.DataProducer)
	}{
		{
			name:    "no plugins",
			plugins: []fwkrhapi.DataProducer{},
		...
		},
		{
			...
			plugins:   []fwkrhapi.DataProducer{pluginFail, pluginDependsOnFail},
			expectErr: true,
			checkFunc: func(t *testing.T, plugins []fwkrhapi.DataProducer) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := executePluginsAsDAG(context.Background(), tc.plugins, &fwkrhapi.InferenceRequest{}, nil)
