...
func TestProduce_PopulatesTokenizedPrompt(t *testing.T) {
	...
	req := &scheduling.InferenceRequest{
		Body: &fwkrhapi.InferenceRequestBody{
			ChatCompletions: &fwkrhapi.ChatCompletionsRequest{
				Messages: []fwkrhapi.Message{{Role: "user", Content: fwkrhapi.Content{Raw: "hi"}}},
			},
		},
	}
	...
	assert.Equal(t, fwkrhapi.ModalityImage, req.Body.TokenizedPrompt.MultiModalFeatures[0].Modality)
}

func TestProduce_SkipsWhenAlreadyPopulated(t *testing.T) {
	existing := &fwkrhapi.TokenizedPrompt{TokenIDs: []uint32{42}}
	p := newTestPlugin(&mockTokenizer{})
	req := &scheduling.InferenceRequest{
		Body: &fwkrhapi.InferenceRequestBody{TokenizedPrompt: existing},
	}
	require.NoError(t, p.Produce(context.Background(), req, nil))
...
func TestProduce_UnsupportedBodyType(t *testing.T) {
	p := newTestPlugin(&mockTokenizer{})
	req := &scheduling.InferenceRequest{
		Body: &fwkrhapi.InferenceRequestBody{}, // no Completions or ChatCompletions
	}
	err := p.Produce(context.Background(), req, nil)
	require.Error(t, err)
...
func TestChatCompletionsToRenderChatRequest(t *testing.T) {
	chat := &fwkrhapi.ChatCompletionsRequest{
		Messages: []fwkrhapi.Message{
			{Role: "system", Content: fwkrhapi.Content{Raw: "You are a helpful assistant."}},
...
func TestChatCompletionsToRenderChatRequest_MultimodalContent(t *testing.T) {
	tests := []struct {
			...
			messages: []fwkrhapi.Message{
				{Role: "user", Content: fwkrhapi.Content{
					Structured: []fwkrhapi.ContentBlock{
						...
						{Type: "image_url", ImageURL: fwkrhapi.ImageBlock{URL: "data:image/png;base64,abc123"}},
					},
				}},
			},
			wantConv: []tokenizerTypes.Conversation{
		...
		},
		{
			...
			messages: []fwkrhapi.Message{
				{Role: "system", Content: fwkrhapi.Content{Raw: "You are a visual analyst."}},
				{Role: "user", Content: fwkrhapi.Content{
					Structured: []fwkrhapi.ContentBlock{
						{Type: "text", Text: "Compare these two images"},
						{Type: "image_url", ImageURL: fwkrhapi.ImageBlock{URL: "data:image/png;base64,img1"}},
						{Type: "image_url", ImageURL: fwkrhapi.ImageBlock{URL: "data:image/png;base64,img2"}},
					},
				}},
			},
			wantConv: []tokenizerTypes.Conversation{
			...
			messages: []fwkrhapi.Message{
				{Role: "user", Content: fwkrhapi.Content{
					Structured: []fwkrhapi.ContentBlock{
						...
						{Type: "image_url", ImageURL: fwkrhapi.ImageBlock{URL: "https://example.com/img.jpg"}},
					},
				}},
				{Role: "assistant", Content: fwkrhapi.Content{Raw: "I see a dog."}},
				{Role: "user", Content: fwkrhapi.Content{Raw: "What breed is it?"}},
			},
			wantConv: []tokenizerTypes.Conversation{
				{Role: "user", Content: tokenizerTypes.Content{
					Structured: []tokenizerTypes.ContentBlock{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			chat := &fwkrhapi.ChatCompletionsRequest{Messages: tt.messages}
			result := ChatCompletionsToRenderChatRequest(chat)
