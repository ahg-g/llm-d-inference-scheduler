...
var _ fwkrhapi.DataProducer = &Plugin{}
...
func (p *Plugin) Produces() map[plugin.DataKey]any {
	return map[plugin.DataKey]any{p.dk: fwkrhapi.TokenizedPrompt{}}
}
...
func (p *Plugin) tokenize(ctx context.Context, request *scheduling.InferenceRequest) (*fwkrhapi.TokenizedPrompt, error) {
	...
	return &fwkrhapi.TokenizedPrompt{
		TokenIDs:           tokenIDs,
...
func ChatCompletionsToRenderChatRequest(chat *fwkrhapi.ChatCompletionsRequest) *tokenizerTypes.RenderChatRequest {
...
func convertMMFeaturesToUpstream(src *tokenization.MultiModalFeatures) []fwkrhapi.MultiModalFeature {
	if src == nil || len(src.MMHashes) == 0 {
		return nil
	}

	var items []fwkrhapi.MultiModalFeature
	for modality, hashes := range src.MMHashes {
		...
		for i := 0; i < n; i++ {
			items = append(items, fwkrhapi.MultiModalFeature{
...
func ConvertMMFeaturesFromUpstream(features []fwkrhapi.MultiModalFeature) (map[string][]string, map[string][]kvblock.PlaceholderRange) {
	if len(features) == 0 {
		return nil, nil
	}
