...
import (
	...
	"github.com/llm-d/llm-d-router/pkg/epp/framework/plugins/requesthandler/parsers"
)
...
var _ fwkrhapi.Parser = &OpenAIParser{}

// OpenAIParser implements the fwkrhapi.Parser interface for OpenAI API
...
func (p *OpenAIParser) ParseRequest(ctx context.Context, body []byte, headers map[string]string) (*fwkrhapi.ParseResult, error) {
	...
	extractedBody.Payload = fwkrhapi.PayloadMap(bodyMap)
	if stream, ok := bodyMap["stream"].(bool); ok && stream {
		extractedBody.Stream = true
	}
	return &fwkrhapi.ParseResult{Body: extractedBody, Skip: false}, nil
}
...
func (p *OpenAIParser) ParseResponse(ctx context.Context, body []byte, headers map[string]string, _ bool) (*fwkrhapi.ParsedResponse, error) {
	if len(body) == 0 {
	...
	return &fwkrhapi.ParsedResponse{Usage: usage}, nil
}

func (p *OpenAIParser) parseStreamResponse(chunk []byte) (*fwkrhapi.ParsedResponse, error) {
	usage := extractUsageStreaming(string(chunk))
	return &fwkrhapi.ParsedResponse{
		Usage: usage,
	}, nil
}
...
func extractRequestBody(rawBody []byte, headers map[string]string) (*fwkrhapi.InferenceRequestBody, error) {
...
func validateChatCompletionsMessages(messages []fwkrhapi.Message) error {
	if len(messages) == 0 {
...
func extractUsage(responseBytes []byte) (*fwkrhapi.Usage, error) {
		...
		if usg["prompt_token_details"] != nil {
			if detailsMap, ok := usg["prompt_token_details"].(map[string]any); ok {
				if cachedTokens, ok := detailsMap["cached_tokens"]; ok {
					usage.PromptTokenDetails = &fwkrhapi.PromptTokenDetails{
...
func extractUsageByAPIType(usg map[string]any, objectType string) fwkrhapi.Usage {
...
func extractUsageStreaming(responseText string) *fwkrhapi.Usage {

	var streamResponse struct {
		Usage    *fwkrhapi.Usage `json:"usage"`
		Response struct {
