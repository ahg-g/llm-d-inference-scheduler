...
func TestParseRequest(t *testing.T) {
	...
	tests := []struct {
		...
		wantErr    string
		wantResult *fwkrhapi.ParseResult
	}{
		{
			...
			wantResult: &fwkrhapi.ParseResult{
				Body: &fwkrhapi.InferenceRequestBody{
					...
					Stream:  true,
					Payload: fwkrhapi.PayloadProto{Message: reqMsg},
				},
				Skip: false,
			},
		},
...
func TestParseResponse(t *testing.T) {
	...
	tests := []struct {
		...
		headers    map[string]string
		wantErr    string
		wantResult *fwkrhapi.ParsedResponse
	}{
		{
			name:       "Empty body",
			body:       []byte{},
		...
		},
		{
			...
			wantResult: &fwkrhapi.ParsedResponse{
				Usage: &fwkrhapi.Usage{PromptTokens: 10, CompletionTokens: 20, TotalTokens: 30},
			},
		},
		{
			name:    "Invalid gRPC frame",
