...
var _ fwkrhapi.Parser = &VertexAIParser{}

// VertexAIParser implements the fwkrhapi.Parser interface for Vertex AI gRPC API
...
func (p *VertexAIParser) ParseRequest(ctx context.Context, body []byte, headers map[string]string) (*fwkrhapi.ParseResult, error) {
	...
	case strings.HasSuffix(path, chatCompletionsMethod):
		...
		inferenceRequestBody.Payload = fwkrhapi.PayloadProto{Message: req}
		return &fwkrhapi.ParseResult{Body: inferenceRequestBody, Skip: parseResult.Skip}, nil
...
func (p *VertexAIParser) ParseResponse(ctx context.Context, body []byte, headers map[string]string, _ bool) (*fwkrhapi.ParsedResponse, error) {
	if len(body) == 0 {
		// Nothing to parse
