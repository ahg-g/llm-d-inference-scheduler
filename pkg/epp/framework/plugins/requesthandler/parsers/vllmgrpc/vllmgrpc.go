...
import (
	...
	grpcutil "github.com/llm-d/llm-d-router/pkg/epp/framework/plugins/requesthandler/parsers/util"
	pb "github.com/llm-d/llm-d-router/pkg/epp/framework/plugins/requesthandler/parsers/vllmgrpc/api/gen"
)
...
var _ fwkrhapi.Parser = &VllmGRPCParser{}

// VllmGRPCParser implements the fwkrhapi.Parser interface for vLLM gRPC.
...
func (p *VllmGRPCParser) ParseRequest(ctx context.Context, body []byte, headers map[string]string) (*fwkrhapi.ParseResult, error) {
	...
	default:
		...
		return &fwkrhapi.ParseResult{Skip: true}, nil
	}
}

// ParseResponse parses the response body and returns a ParsedResponse
func (p *VllmGRPCParser) ParseResponse(ctx context.Context, body []byte, headers map[string]string, endofStream bool) (*fwkrhapi.ParsedResponse, error) {
	logger := log.FromContext(ctx)
	...
	if err := toGenerateResponse(body, resp); err != nil || resp.Response == nil {
		...
		if err := toEmbedResponse(body, embedResp); err == nil && (len(embedResp.Embedding) > 0 || embedResp.PromptTokens > 0) {
			...
			result := &fwkrhapi.ParsedResponse{}
			if embedResp.PromptTokens > 0 {
				result.Usage = &fwkrhapi.Usage{
...
func requestControlUsage(promptToken, completionToken, cachedToken int) *fwkrhapi.Usage {
	return &fwkrhapi.Usage{
		PromptTokens:     promptToken,
		CompletionTokens: completionToken,
		TotalTokens:      promptToken + completionToken,
		PromptTokenDetails: &fwkrhapi.PromptTokenDetails{
...
func convertToInferenceRequestBody(pbReq *pb.GenerateRequest) (*fwkrhapi.InferenceRequestBody, error) {
	var body *fwkrhapi.InferenceRequestBody
	...
	case *pb.GenerateRequest_Text:
		body = &fwkrhapi.InferenceRequestBody{
			Completions: &fwkrhapi.CompletionsRequest{
				Prompt: fwkrhapi.Prompt{Raw: pbReq.GetText()},
			},
			Payload: fwkrhapi.PayloadProto{Message: pbReq},
		}
	case *pb.GenerateRequest_Tokenized:
		...
		body = &fwkrhapi.InferenceRequestBody{
			Completions: &fwkrhapi.CompletionsRequest{
				Prompt: fwkrhapi.Prompt{TokenIDs: copiedTokenIDsInt},
			},
			Payload: fwkrhapi.PayloadProto{Message: pbReq},
...
func convertMultiModalFeatures(mmInputs *pb.MultimodalInputs) []fwkrhapi.MultiModalFeature {
	if mmInputs == nil {
		return nil
	}
	...
	features := make([]fwkrhapi.MultiModalFeature, 0, len(placeholders))
	for i, placeholder := range placeholders {
		...
		feature := fwkrhapi.MultiModalFeature{
			Modality: fwkrhapi.ModalityImage,
			Hash:     hash,
		}
		if placeholder != nil {
...
func convertEmbedToInferenceRequestBody(pbReq *pb.EmbedRequest) (*fwkrhapi.InferenceRequestBody, error) {
	var body *fwkrhapi.InferenceRequestBody
	if pbReq.Tokenized != nil {
		...
		body = &fwkrhapi.InferenceRequestBody{
			Embeddings: &fwkrhapi.EmbeddingsRequest{
				Input: fwkrhapi.EmbeddingsInput{
					TokenIDs: tokenIDs,
				},
			},
			Payload: fwkrhapi.PayloadProto{Message: pbReq},
